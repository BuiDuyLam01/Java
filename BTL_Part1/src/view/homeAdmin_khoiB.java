...
import control.Connect_KhoiB;
import model.ThiSinhB;


public class homeAdmin_khoiB extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					homeAdmin_khoiB frame = new homeAdmin_khoiB();
	 ...
	 * Create the frame.
	 */
	public homeAdmin_khoiB(String user) {
		setTitle("Quản lí khối B");
		...
		btnKhoiB.setBackground(new Color(32, 178, 170));
		...
		btnTimKiem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				showData(Connect_KhoiB.findbySBD(ts));
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"SBD", "H\u1ECD v\u00E0 t\u00EAn", "N\u0103m sinh", "Gi\u1EDBi t\u00EDnh", "To\u00E1n", "Sinh", "H\u00F3a", "\u0110i\u1EC3m UT", "T\u1ED5ng \u0111i\u1EC3m"
			}
		));
		...
		JLabel lblNewLabel_1_1_1_1_1_2 = new JLabel("Điểm Sinh");
		...
		btnTao.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ThiSinhB ts = new ThiSinhB();
				ts.setSoBaoDanh(txtSBD.getText());
				if(Connect_KhoiB.kiemtratontai2(ts)==true) {
					JOptionPane.showMessageDialog(null, "Sinh viên này đã có tài khoản!");
				} else {
					Connect_KhoiB.Add_User(ts);;
	...
	public void showData(List<ThiSinhB> thisinhl) {
		...
		dsts.forEach((thisinh) -> {
			...
			tableModel.addRow(new Object[] { thisinh.getSoBaoDanh(), thisinh.getHoVaTen(), thisinh.getNamSinh(), gioitinh,
					thisinh.getDiemToan(), thisinh.getDiemSinh(), thisinh.getDiemHoa(), thisinh.getDiemUuTien(),
					thisinh.getDiemToan()+thisinh.getDiemSinh()+thisinh.getDiemHoa()+thisinh.getDiemUuTien()});
		});
	}
}
