...
import control.Connect_KhoiC;
import model.ThiSinhC;


public class homeAdmin_khoiC extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					homeAdmin_khoiC frame = new homeAdmin_khoiC();
	 ...
	 * Create the frame.
	 */
	public homeAdmin_khoiC(String user) {
		setTitle("Quản lí khối C");
		...
		btnKhoiC.setBackground(new Color(32, 178, 170));
		...
		JLabel lblNewLabel = new JLabel("Quản lí khối B");
		...
		JLabel lblNewLabel_1_1_1_1_1 = new JLabel("Điểm Văn");
		...
		btnLuu.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ts.setDiemVan(Float.parseFloat(txtToan.getText()));
				ts.setDiemSu(Float.parseFloat(txtSinh.getText()));
				ts.setDiemDia(Float.parseFloat(txtHoa.getText()));
				ts.setDiemUuTien(Float.parseFloat(txtDUT.getText()));
				
				if(Connect_KhoiC.kiemtratontai(ts)==true) {
					JOptionPane.showMessageDialog(null, "Sinh viên này đã tồn tại!");
				} else {
					Connect_KhoiC.insert(ts);
					JOptionPane.showMessageDialog(null, "Lưu thông tin thành công!");
					showData(Connect_KhoiC.findAll());
				}
			}
		});
		...
		btnTimKiem.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				showData(Connect_KhoiC.findbySBD(ts));
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				txtSinh.setText(model_table.getValueAt(selectedIndex, 5).toString());
		...
		table.setModel(new DefaultTableModel(
			...
			new String[] {
				"SBD", "H\u1ECD v\u00E0 t\u00EAn", "N\u0103m sinh", "Gi\u1EDBi t\u00EDnh", "V\u0103n", "S\u1EED", "\u0110\u1ECBa", "\u0110i\u1EC3m UT", "T\u1ED5ng \u0111i\u1EC3m"
			}
		));
		...
		JLabel lblNewLabel_1_1_1_1_1_2 = new JLabel("Điểm Sử");
		...
		txtSinh.setColumns(10);
		txtSinh.setBounds(186, 359, 83, 26);
		contentPane.add(txtSinh);

		JLabel lblNewLabel_1_1_1_1_1_3 = new JLabel("Điểm Địa");
		...
		btnTao.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ThiSinhC ts = new ThiSinhC();
				ts.setSoBaoDanh(txtSBD.getText());
				if(Connect_KhoiC.kiemtratontai2(ts)==true) {
					JOptionPane.showMessageDialog(null, "Sinh viên này đã có tài khoản!");
				} else {
					Connect_KhoiC.Add_User(ts);;
	...
	public void showData(List<ThiSinhC> thisinhl) {
		...
		dsts.forEach((thisinh) -> {
			...
			tableModel.addRow(new Object[] { thisinh.getSoBaoDanh(), thisinh.getHoVaTen(), thisinh.getNamSinh(), gioitinh,
					thisinh.getDiemVan(), thisinh.getDiemSu(), thisinh.getDiemDia(), thisinh.getDiemUuTien(),
					thisinh.getDiemVan()+thisinh.getDiemSu()+thisinh.getDiemDia()+thisinh.getDiemUuTien()});
		});
	}
}
